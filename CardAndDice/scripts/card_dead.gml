...
{
    if target_list = "enemy" 
    {ds_list_replace(obj_GAME_SYSTEM_MANAGER.enemy_list,target_pos,undefined)}
    else 
    {ds_list_replace(obj_GAME_SYSTEM_MANAGER.player_list,target_pos,undefined)}
    var ID = instance_create(ins.x,ins.y,obj_reverse_summoning_animation);
    ID.portrait = ins.card_portrait
    send_log(ins.hangul_name+"는 역소환 되었습니다.",true)
    ds_stack_push(global.instance_recycle_bin,ins)
